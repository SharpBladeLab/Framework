...
class AlipaySubmit {
	...
	function buildForm($para_temp, $gateway, $method, $button_name, $aliapy_config) {
		...
		$sHtml = $sHtml."<head>";
		$sHtml = $sHtml."<meta http-equiv=\"Content-Type\" content=\"text/html; charset=".$aliapy_config['input_charset']."\" />";
		$sHtml = $sHtml."</head><body>";
		$sHtml = $sHtml."<form id='alipaysubmit' name='alipaysubmit' action='".$gateway."_input_charset=".trim(strtolower($aliapy_config['input_charset']))."' method='".$method."'>";
