 ...
 *
 * $Id: AdvModel.class.php 524 2013-07-31 02:26:10Z wgw $
 *
 ...
 */
 class AdvModel extends Model { 
    ...
    protected function _before_update(&$data,$options='') {
        ...
        /* 检查乐观锁 */
        if(!$this->checkLockVersion($data,$options)) return false;
        ...
        if($this->optimLock && isset($_SESSION[$identify])) {
            ...
            if(isset($curr_version)) {
                if($curr_version>0 && $lock_version != $curr_version) {
                    /* 记录已经更新 */
                    $this->error = Helper::createLanguage('_RECORD_HAS_UPDATE_');
                    return false;
        ...
        switch($type) {
            ...
            default:
                ...
                if(class_exists($type))
                    ...
                    Helper::createException(Helper::createLanguage('_CLASS_NOT_EXIST_').':'.$type);
        }
    }
        ...
        if(!empty($this->blobFields)) {
			...
			if(empty($field)) {
                foreach ($this->blobFields as $field){
                    ...
                    $data[$field] =  Helper::createTempFile($identify);
                }
                return $data;
            } else {
                $identify   =   $this->name.'/'.$id.'_'.$field;
                return Helper::createTempFile($identify);
            }
        }
    }
    ...
    protected function delBlobFields(&$data,$field='') {
        if(!empty($this->blobFields)) {
            ...
            if(empty($field)) {
                foreach ($this->blobFields as $field){
                    $identify   =   $this->name.'/'.$id.'_'.$field;
                    Helper::createTempFile($identify,null);
                }
            } else {
                $identify   =   $this->name.'/'.$id.'_'.$field;
                Helper::createTempFile($identify,null);
    ...
    protected function lazyWrite($guid,$step,$lazyTime) {
        if(false !== ($value = Helper::createTempFile($guid))) {
			
			/* 存在缓存写入数据 */
            if(time()>Helper::createTempFile($guid.'_time')+$lazyTime) {
                
				/*  延时更新时间到了，删除缓存数据 并实际写入数据库 */
                Helper::createTempFile($guid,NULL);
                Helper::createTempFile($guid.'_time',NULL);
                return $value+$step;
				
            } else {
                /* 追加数据到缓存 */
                Helper::createTempFile($guid,$value+$step);
                return false;
            }
			
        } else { 
			/* 没有缓存数据 */
            Helper::createTempFile($guid,$step);
            /*  计时开始 */
            Helper::createTempFile($guid.'_time',time());
            return false;
        }
    }
    ...
    public function addConnect($config,$linkNum=NULL) {
        ...
        if(NULL === $linkNum && is_array($config)) {
            ...
            foreach ($config as $key=>$val) {
                $this->_db[$key]  = DataBase::getInstance($val);
			}
    ...
    public function getPartitionTableName($data=array()) {
        ...
        if(isset($data[$this->partition['field']])) {
            ...
            switch($this->partition['type']) {
                ...
                case 'year':
                    /* 按照年份分表 */
                    if(!is_numeric($field)) $field = strtotime($field);                    
            ...
            /* 当设置的分表字段不在查询条件或者数据中.进行联合查询，必须设定 partition['num'] */
            $tableName  =   array();
