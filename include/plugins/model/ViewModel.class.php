 ...
 * 
 * $Id: ViewModel.class.php 524 2013-07-31 02:26:10Z wgw $
 *
 ...
 class ViewModel extends Model 
 {
    ...
    public function getTableName()
    {
        if(empty($this->trueTableName)) {
            ...
            foreach ($this->viewFields as $key=>$view) {
				...
				/* 获取数据表名称 */
                $class  =   $key.'Model';
                $Model  =  class_exists($class)?new $class(): Helper::createModel($key, '', true);
    ...
    private function _checkFields($name,$fields) {
		if(false !== $pos = array_search('*',$fields)) {
			/* 定义所有字段 */
            $fields  =  array_merge($fields, Helper::createModel($name, '', true)->getDbFields());
            unset($fields[$pos]);
        }
        return $fields;
    }
