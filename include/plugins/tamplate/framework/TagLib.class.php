 ...
 *
 * $Id: TagLib.class.php 515 2013-07-30 08:59:14Z wgw $
 *
 * Copyright (C) 2007-2011 Tiwer Developer Framework. All Rights Reserved.
 */
 class TagLib extends Framework
 {
    ...
    public function __construct()
    {
        ...
        $this->tpl       = Framework::instance('ThinkTemplate');//ThinkTemplate::getInstance();
        $this->_initialize();
        $this->load();
    }
    ...
    public function parseXmlAttr($attr,$tag)
    {
        ...
        if(!$xml) {
            Helper::createException(Helper::createLanguage('_XML_TAG_ERROR_').' : '.$attr);
        }
    ...
    /**
     * 用于标签属性里面的特殊模板变量解析格式 以 Think. 打头的变量属于特殊模板变量
     *
     * @access public
     *
    ...
    public function parseThinkVar($varStr) {
        ...
        if(count($vars)>=3) {
            ...
            switch($vars[1]){
                ...
                case 'LANG':    $parseStr = 'Helper::createLanguage("'.$vars[2].'")';break;
                case 'CONFIG':  $parseStr = 'config("'.$vars[2].'")';break;
            }
        } else if(count($vars)==2) {
            switch($vars[1]) {
                case 'NOW':      $parseStr = "date('Y-m-d g:i a',time())";break;
                case 'VERSION':  $parseStr = 'THINK_VERSION';break;
                case 'TEMPLATE': $parseStr = 'config("TMPL_FILE_NAME")';break;
                case 'LDELIM':   $parseStr = 'config("TMPL_L_DELIM")';break;
                case 'RDELIM':   $parseStr = 'config("TMPL_R_DELIM")';break;
