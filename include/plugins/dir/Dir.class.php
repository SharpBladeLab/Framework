 ...
 * @version     $Id: Dir.class.php 524 2013-07-31 02:26:10Z wgw $
 *
 * DirectoryIterator实现类 PHP5以上内置了DirectoryIterator类
 */
 class Dir extends Plugin implements IteratorAggregate
 {
	...
	function listFile($pathname, $pattern = '*') {
		...
		if(!isset($_listDirs[$guid])) {
			...
			$cmp_func = create_function('$a,$b', '$k = "isDir"; if($a[$k] == $b[$k]) return 0; return $a[$k]>$b[$k]?-1:1;');
 ...
 /* DirectoryIterator类不存地情况时DirectoryIterator继承Dir */ 
