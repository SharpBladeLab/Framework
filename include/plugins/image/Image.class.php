 ...
 */
 class Image extends Plugin
 {
     ...
     * @return string
     */
    static function buildImageVerify($length=4,$mode=2,$type='png',$width=125 ,$height=30, $verifyName='verify') {		
		...
		/* 背景色 */
		$backColor = imagecolorallocate($im, 44, 203, 244);   
		...
		/* 获取字体文件 */
		$fonts = glob( DATAS_PATH .SEP.'fonts'.SEP.'*.ttf');
				
		/* 写入文字 */
		for($i=0; $i<$length; $i++) {
			
			/* 字体颜色 */			
			$stringColor = imagecolorallocate($im, mt_rand(0,120), mt_rand(0,120), mt_rand(0,120));
			
			if( count($fonts) > 0) {
				
				$font = $fonts[mt_rand(0, count($fonts)-1 )];				
				/* ttf 字体方式写入到图片上 */
				imagettftext($im, 14, rand(0,30), $i*30+10, mt_rand(18,25), $stringColor, $font, $randval{$i});
			
			} else {
				/* 简单方式写入图片 */
				imagestring($im, 5, $i*29+5, mt_rand(1,13), $randval{$i}, $stringColor);
			}
		}		
