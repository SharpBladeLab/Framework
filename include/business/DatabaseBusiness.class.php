 ...
 *
 * $Id: DatabaseBusiness.class.php 515 2013-07-30 08:59:14Z wgw $
 *
 * Copyright (C) 2007-2010 Tiwer Developer Team. All Rights Reserved.
 */
 class DatabaseBusiness extends Business {

	public function getTableList() {
		return Helper::createModel('', '', true)->query('SHOW TABLE STATUS LIKE "__TABLE__%"');
	}
	...
	public function getTableSql($table, $startfrom=0, $filesize,$currentsize,$complete=true) {
		...
		$tablefields = Helper::createModel('', '', true)->query('SHOW FULL COLUMNS FROM '.$table);
		if(!$tablefields)
			return false;

		if($startfrom == 0)  {
			$createtable	= Helper::createModel('', '', true)->query('SHOW CREATE TABLE '.$table);
