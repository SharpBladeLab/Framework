 ...
 * @version     $Id: SessionFile.class.php 516 2013-07-30 09:02:02Z wgw $
 *
 * Sessiony文件管理类
 */ 
 /* 常量定义 */
 ...
 class SessionFile extends SessionBase {
	...
	public function init() {		
        ...
        Session::setCookieDomain(config('COOKIE_DOMAIN'));
		SessionFile::localName(APP_NAME);
        SessionFile::path(SESSION_PATH);      
        SessionFile::name(config('SESSION_NAME'));       
        SessionFile::setCallback(config('SESSION_CALLBACK'));		
	}
	...
	public function start() {
        ...
        Session::setExpire(config('SESSION_EXPIRE'));
	}
     ...
     */
    static function loadSessionFile($id) {
    	
    	$res = array();	
    	
    	/* 设置会话ID  */
    	session_id($id);
		$string = file_get_contents(self::getFilename());
    ...
    static function setExpire($time, $add = false)  {
        ...
        if ($add) {
            if (!isset($_SESSION['__HTTP_Session_Expire_Tiwer'])) {
                $_SESSION['__HTTP_Session_Expire_Tiwer'] = time() + $time;
            }
    ...
    static function sessionValidThru() {
        if (!isset($_SESSION['__HTTP_Session_Idle_Tiwer']) || !isset($_SESSION['__HTTP_Session_Idle'])) {
            return 0;
        } else {
            return $_SESSION['__HTTP_Session_Idle_Tiwer'] + $_SESSION['__HTTP_Session_Idle'];
        }
    }
    ...
    static function isIdle() {
        if (isset($_SESSION['__HTTP_Session_Idle_Tiwer']) && (($_SESSION['__HTTP_Session_Idle_Tiwer'] + $_SESSION['__HTTP_Session_Idle']) < time())) {
            return true;
        } else {
            return false;
        }
    ...
    static function updateIdle() {
        $_SESSION['__HTTP_Session_Idle_Tiwer'] = time();
    }
    ...
    static function getFilename() {
		return Session::path(). SEP .'sess_'.session_id();
    }	
}
