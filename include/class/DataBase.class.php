 ...
 {    
    ...
    public function factory( $db_config = '' ) {
        ...
        if( empty($db_config['dbms']) ) {
            Helper::createException(Helper::createLanguage('_NO_DB_CONFIG_'));
		}
        ...
        if( class_exists($dbClass) ) {
            ...
            /* 类没有定义 */
            Helper::createException(Helper::createLanguage('_NOT_SUPPORT_DB_').': ' . $db_config['dbms']);
        }
                    ...
                    if(is_array($val)) {
                        ...
                        if(is_string($val[0])) {
                            if(preg_match('/^(EQ|NEQ|GT|EGT|LT|ELT|NOTLIKE|LIKE)$/i',$val[0])) { 
                                ...
                                Helper::createException(Helper::createLanguage('_EXPRESS_ERROR_').':'.$val[0]);
                            }
                        }else {
                            $count = count($val);
