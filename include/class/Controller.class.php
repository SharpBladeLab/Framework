 ...
 abstract class Controller extends Framework {
    ...
    */
    public function __construct() {
        ...
        /* 实例化视图类 */
        $this->view = Framework::instance('View');		
		
		/* 实例化日志类 */
		$this->log  = Helper::createBusiness('Logs');
   ...
   /**
    ...
    protected function isAjax() {
        ...
        if(!empty($_POST[config('VAR_AJAX_SUBMIT')]) || !empty($_GET[config('VAR_AJAX_SUBMIT')])) {
            /* 判断Ajax方式提交 */
            return true;
		}
    ...
    public function __call($method,$parms) {
        ...
        if( 0 === strcasecmp($method,ACTION_NAME)) {
            /* 检查扩展操作方法 */
            $_action = config('_actions_');
            if($_action) {
			
                /* 'module:action'=>'callback' */
                if(isset($_action[CONTROLLER_NAME.':'.ACTION_NAME])) {
                    $action  =  $_action[CONTROLLER_NAME.':'.ACTION_NAME];
            ...
            if(method_exists($this,'_empty')) {
                ...
                if(file_exists_case(config('TMPL_FILE_NAME'))) {
                    $this->display();
					
                } else {
                    /* 抛出异常 */
                    Helper::createException(Helper::createLanguage('_ERROR_ACTION_').ACTION_NAME);
				}
            }
     ...
     * @return void
     */
    protected function echoMessage($message,$ajax=false)  {
        $this->_dispatch_jump_noexit($message,1,$ajax);
    }
    ...
    protected function ajaxReturn($data,$info='',$status=1,$type='') {
        ...
        if(strtoupper($type)=='JSON') {
            ...
            exit(Helper::createPlugin('Xml')->encode($result));
    ...
    protected function redirect($url, $params = array(), $delay = 0, $msg = '' ) {        
		if(config('LOG_RECORD')) Log::save();		
        $url = Helper::createLink($url,$params);
        redirect($url,$delay,$msg);
    }

    /**
     * 默认跳转操作,支持错误导向和正确跳转
     * 调用模板显示 默认为public目录下面的success页面.提示页面为可配置 支持模板标签
     *
     * @param string  $message 提示信息
     ...
     * @return void
     */
    private function _dispatch_jump_noexit($message, $status = 1, $ajax = false) {	
		global $dc;
        ...
        if($status) {
			...
			$this->display(COMMON_TPL_PATH.'&sendsms');
	...
	public function setSessionAndCookie($uid, $uname, $email, $remember = false) {
		...
		/* 是否记着登录 */
		$remember ? 
			cookie('LOGGED_USER', base64_encode('digcity_tiwer_. '. $uid), (3600*24*365) ) : 
			cookie('LOGGED_USER', base64_encode('digcity_tiwer_.' . $uid), (3600*2));
	}
 }
